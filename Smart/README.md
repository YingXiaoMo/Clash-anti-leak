...
（Action 自动创建） CSV 数据下载的临时目标目录。

必填

models/

（脚本自动创建） 存放最终生成的模型文件 Model.bin。
...
重要说明： 在 GitHub Actions 运行时，data/ 目录由工作流自动创建，models/ 目录由 Python 脚本自动创建。以下要求针对本地测试和文件结构完整性。
...
（Action 自动创建） 包含您的 CSV 历史数据文件。

models/

（脚本自动创建） 用于存放最终的 Model.bin 文件。
...
请在工作流文件中的 steps: 列表里，紧跟在 actions/checkout 步骤之后，添加如下步骤：
      ...
      - name: 从 Google Cloud Storage 下载历史数据 (gsutil)
        run: |
          # 目录创建已包含在 gsutil 命令中，但我们在此处明确显示其目标
          echo "创建数据目录..."
          mkdir -p data 
...
请在工作流文件中的 steps: 列表里，紧跟在 actions/checkout 步骤之后，添加如下步骤：
      ...
      - name: 配置 Rclone 认证
        run: |
          ...
          # ⚠️ 注意：这里假设您的 rclone.conf 中只有一个远程配置
