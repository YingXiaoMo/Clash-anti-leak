...
1.  **V3 兼容：** 最终模型使用 `smart-store-creator` 库进行 V3 二进制编码，可以直接被 Mihomo 内核使用。
2.  **Go 源码特征解析：** Python 脚本自动解析 Mihomo Go 源码中定义的特征顺序，确保模型训练的特征与实际运行时使用的特征一致。
3.  **数据清洗与加载：** 自动遍历数据文件，执行数据加载、缺失值处理和清洗。
4.  **LightGBM 模型训练：** 使用 LightGBM 框架进行高效的模型训练。
5.  **GitHub Actions 自动化：** 自动执行整个训练流程。
6.  **最终产物发布（Release）：** 训练成功的模型文件（`Model.bin`）将通过 GitHub Release 发布，**不会**提交回仓库的代码或 `models/` 目录中。
